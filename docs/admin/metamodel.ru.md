...
| **Качество данных** | Насколько это поле влияет на оценку качества данных: **Игнорировать** (исключено), **Обычный**, **Важный** или **Критический** |
...
Оценка **качества данных** карточки — это взвешенная мера её полноты. Каждое поле учитывается в оценке в соответствии со своей важностью **Качество данных** (задаётся в редакторе полей): **Игнорировать** исключает поле из оценки, а **Обычный**, **Важный** и **Критический** придают ему всё больший вес.

Помимо полей, в оценку вносят вклад четыре встроенных фактора: **Описание**, **Жизненный цикл** (задана ли хотя бы одна дата жизненного цикла), а также любые применимые к типу **Обязательные связи** и **Обязательные теги**. Каждый из них можно настроить или исключить на панели **Качество данных** внизу редактора макета типа карточки с помощью того же выбора «Игнорировать / Обычный / Важный / Критический». Например, задайте **Жизненный цикл** как *Игнорировать* для типа, карточки которого закономерно никогда не содержат дат, чтобы они не получали штраф.

Изменение любой настройки важности немедленно пересчитывает оценку всех существующих карточек этого типа. Новые поля по умолчанию имеют значение *Обычный*, поэтому учитываются в оценке сразу после добавления.
