...
除属性外，调查还可以请受访者保持卡片**关系**的最新状态。在**字段**步骤中，**关系**部分会列出目标卡片类型可能具有的所有关系（双向），例如对于应用程序：*支持 → IT 组件* 和 *被使用 ← 组织*。为每个选定的关系选择一个操作：

- **维护** — 受访者可看到当前关联的卡片，并可通过搜索选择器添加或移除关联。
- **确认** — 受访者只需确认当前关联正确，或关闭开关以提议更改。

应用此类响应时，Turbo EA 会添加新的关联并移除受访者删除的关联。该更改会像手动关系编辑一样记录在卡片的历史记录中。
